...
VALID_CONFIG_TYPE = {"llama", "qwen2", "qwen2_vl", "qwen2_5_vl", "qwen3", "qwen3_moe", "deepseek_v3", "paligemma"}
...
class FlopsCounter:
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.estimate_func = {
            ...
            "paligemma": self._estimate_paligemma_flops,
        }
        self.config = config
    ...
    def _estimate_paligemma_flops(self, tokens_sum, batch_seqlens, delta_time):
        hidden_size = self.config.hidden_size
        ...
        head_dim = getattr(self.config.text_config, "head_dim", self.config.hidden_size // self.config.text_config.num_attention_heads)
        q_size = num_attention_heads * head_dim
        k_size = num_key_value_heads * head_dim
        v_size = num_key_value_heads * head_dim

        # non-attn per layer parm
        # Qwen2/LLama/Gemma use SwiGelu, gate, having up and down linear layer in mlp
        mlp_N = hidden_size * intermediate_size * 3
        attn_linear_N = hidden_size * (q_size + k_size + v_size + num_attention_heads * head_dim)
        emd_and_lm_head_N = vocab_size * hidden_size * 2
        # non-attn all_layer parm
        ...
        # all_layer & all_token fwd & bwd flops
        flops_txt = dense_N_flops + attn_qkv_flops

        # 2) SigLIP Vision Encoder FLOPs （粗略公式）
        vcfg = self.config.vision_config
        Ntok = vcfg.num_image_tokens * 3
        L    = vcfg.num_hidden_layers
        d    = vcfg.hidden_size
        # ViT 每层 ~ 2*Ntok^2*d   （QK^T、softmax、V） + 4*Ntok*d^2 （Q,K,V投影&FFN）; ×2 for bwd
        flops_vision = 2 * L * (2 * (Ntok**2) * d + 4 * Ntok * (d**2))

        # 3) Projector FLOPs（fwd+bwd）
        proj_dim = self.config.projection_dim  # =2048
        flops_proj = 6 * Ntok * d * proj_dim   # W*d*proj_dim * 2 （fwd+bwd）≈

        total_flops = flops_txt + flops_vision + flops_proj
        return (total_flops / delta_time) / 1e12  # TFLOPs/s
