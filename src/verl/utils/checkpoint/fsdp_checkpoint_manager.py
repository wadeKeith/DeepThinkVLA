    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if "hf_model" in self.checkpoint_contents:
            ...
            if self.rank == 0:
                ...
                elif 'DeepThinkVLA' in model_config.architectures[0]:
                    from sft.modeling_deepthinkvla import DeepThinkVLA
                    auto_model_cls = DeepThinkVLA
                else:
                    raise NotImplementedError(f"Unknown architecture {model_config['architectures']}")

                with init_empty_weights():
                    save_model = auto_model_cls._from_config(model_config, torch_dtype=torch.bfloat16)
